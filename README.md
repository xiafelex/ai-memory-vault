# AI Memory Vault

一个从零开始的、基于 Git 的个人 context 与 AI 工作记忆仓库。

它解决的是这件事：

- 你和不同 AI 模型的重要对话，可以被结构化保存到仓库里
- 每次切换模型时，都能导出一份“与你有关的上下文”
- 你的重要资料、工作档案、研发计划和风格证据，可以被持续整理成主题主档案
- 这份上下文不依赖某一个平台，核心资产是你自己的 Git 仓库

## 设计原则

- 数据归你：对话保存在本地 Markdown 和 JSON 中，再同步到 GitHub
- 可读可检索：原始对话、资料抽取文本、摘要、证据和主题档案都尽量用 Markdown 管理
- 模型无关：任何模型只要能读取文本，就能使用这套上下文
- 主题优先：新模型优先读主题主档案，不必每次遍历全部历史会话
- 先简单后增强：先做本地可用版本，再加自动化、语义检索、向量数据库

## 仓库结构

```text
memory/
  profiles/
    user.md                 # 你的长期画像
    preferences.md          # 偏好、风格、工作方式
    conversation_tags.md    # 会话标签模板
    domain_registry.md      # 固定主题索引
  domains/
    career/
    collaboration/
    management/
    strategy/
    technology/
  conversations/
    2026/
      2026-04-06-my-first-session/
        meta.json
        transcript.md
        summary.md
        evidence.md
        open_questions.md
        domain_updates.md
  snapshots/
    active_context.md       # 给新模型直接喂的上下文
  imports/
    ...                     # 从 PDF 等资料抽取出的文本
Felex 资料/
  ...                       # 原始资料、表格、红头文件、研发计划等，不是所有原始文件都必须长期入库
scripts/
  extract_pdf_text.swift
src/
  ai_memory/
    cli.py
    storage.py
templates/
  user.md
  preferences.md
```

## 当前能力

- 初始化记忆仓库目录
- 新建一次重要对话记录
- 按主类别自动补齐会话标签
- 自动生成 `meta.json`
- 自动生成“结论层 / 证据层 / 问题层”文件骨架
- 增加固定主题主档案层，避免每次遍历全部对话
- 支持从 PDF 文件或资料文件夹抽取文本
- 支持把 Excel、Word、PDF 等资料作为证据来源，整理到 `memory/imports/` 和对应主题档案
- 根据已有摘要与长期资料导出 `active_context.md`
- 方便你提交到 Git / GitHub

## 快速开始

### 1. 初始化仓库

```bash
git init
python3 -m src.ai_memory.cli init
```

### 2. 编辑你的长期信息

先填写这两个文件：

- `memory/profiles/user.md`
- `memory/profiles/preferences.md`

它们是所有模型共享的长期背景。

### 3. 创建一条重要对话

```bash
python3 -m src.ai_memory.cli add \
  --title "构建 AI 记忆 Git 框架" \
  --model "Codex / GPT-5" \
  --category thinking \
  --tags ai-memory,github,knowledge-base
```

创建后会生成一个目录，建议这样使用：

- `transcript.md`：完整原始对话
- `summary.md`：你和 AI 共同确认后的结论
- `evidence.md`：保留关键原话、依据、原始判断
- `open_questions.md`：后续还需要追问、核实、校准的问题
- `domain_updates.md`：这次对话是否需要更新固定主题主档案

固定主题主档案放在：

- `memory/domains/`

建议把跨多次对话仍然稳定的重要内容，逐步沉淀到这里，例如：

- 工作经历
- 角色定义
- 季度研发管理计划
- 长期目标
- AI 协作期待
- `/commit` 工作流
- 风格校准场景
- 表达风格
- 管道数字化主线

主类别目前支持：

- `tech`
- `management`
- `expression`
- `thinking`

标签模板见：

- `memory/profiles/conversation_tags.md`

### 4. 导出给新模型的上下文

```bash
python3 -m src.ai_memory.cli build-context
```

输出文件：

- `memory/snapshots/active_context.md`

把它发给新的 AI 模型，作为你的长期上下文起点。

### 5. 从 PDF 资料抽取文本

如果你把 PDF 放进某个文件夹，可以这样批量抽取：

```bash
python3 -m src.ai_memory.cli extract-pdf --input "Felex 资料"
```

如果只抽一个 PDF：

```bash
python3 -m src.ai_memory.cli extract-pdf --input "Felex 资料/某个文件.pdf"
```

抽取结果会写到：

- `memory/imports/`

这样后续我就可以直接读取这些抽取后的文本，而不是每次重新做 PDF 解析。

### 6. 资料导入和证据管理

原始资料通常先放在：

- `Felex 资料/`

抽取、转换或整理后的文本证据放在：

- `memory/imports/`

如果资料会长期影响用户画像、研发管理、表达风格或技术路线，再把稳定结论同步到：

- `memory/profiles/`
- `memory/domains/`
- `memory/conversations/`

注意：

- 压缩包、临时文件、无必要重复文件不建议入库
- `memory/imports/` 是证据来源层，不是最终结论层
- `memory/domains/` 才是跨对话复用的主题主档案层

## 推荐工作流

每次“重要对话”结束后做这几件事：

1. 新建一条会话
2. 粘贴原始对话到 `transcript.md`
3. 把关键原话和依据记到 `evidence.md`
4. 把可复用的信息提炼到 `summary.md`
5. 把还没完全确认的问题记到 `open_questions.md`
6. 检查主类别和标签是否准确
7. 如果这次对你画像有长期影响，就同步更新：
   - `memory/profiles/user.md`
   - `memory/profiles/preferences.md`
8. 重新构建上下文
9. `git add . && git commit -m "Add AI memory: ..."`

如果这次不是纯对话，而是新增了一批资料，例如研发计划、课题奖励、红头文件、论文或修改意见，建议改成：

1. 先读取原始资料
2. 把可读取文本导入到 `memory/imports/`
3. 在当前会话中记录 `summary.md` 和 `evidence.md`
4. 判断是否更新 `memory/domains/` 中的主题主档案
5. 重新构建 `active_context.md`
6. 再提交和推送

## `/commit` 的实际含义

`/commit` 不是保存所有聊天，而是把当前主题段中“值得长期保留的新增内容”写回记忆系统。

默认单位不是一问一答，而是：

- 当前聊天线程里
- 围绕同一主题连续展开
- 已经形成稳定增量的这一整段协作

也就是：

- 一个主题，一个 commit

更适合 `/commit` 的内容：

- 新的稳定结论
- 新的关键原话
- 新的长期偏好或工作方法
- 需要更新固定主题主档案的内容

不必 `/commit` 的内容：

- 普通来回讨论
- 临时试错
- 没有形成长期增量的聊天

更完整的规则见：

- `memory/domains/collaboration/commit_workflow.md`

当用户新增资料后说 `/commit`，默认不是只总结聊天，还要检查是否需要同步：

- 资料导入结果
- 证据层记录
- 主题主档案
- 最新 active context

这种模式适用于研发计划、课题奖励、技术成果材料、风格校准材料等。

## 风格校准场景

如果你是在持续给 AI 投喂论文、提案、视频稿、PPT、AI 润色前后版本，目的是逐步校准你的个人风格，可以把它视为一个固定场景。

- 主类别通常是 `expression`
- 常用补充标签：
  - `style-calibration`
  - `paper-style`
  - `policy-style`
  - `bid-video-style`
  - `ppt-style`
  - `ai-revision-comparison`
  - `review-comment-style`

使用这个场景时，建议同时区分 4 层：

- 场景自带结构
- 用户个人语言风格
- 用户修改标准
- AI 润色偏差

对应规则见：

- `memory/domains/collaboration/style_calibration.md`

## 建议记录什么

优先记录高价值对话：

- 你的长期目标
- 正在做的项目
- 技术中心研发管理计划、课题奖励和项目验证证据
- 个人偏好与禁忌
- 沟通风格
- 常用工具链
- 已经讨论过的重要决策
- 不希望每次重复解释的背景

## 下一步扩展

这个仓库已经能作为第一版“可迁移记忆层”使用。下一步可以继续加：

- 自动从聊天导出 Markdown
- GitHub Actions 自动构建上下文
- 本地全文检索
- 语义检索 / 向量数据库
- MCP / API 接口，让别的 AI 工具直接调用

## GitHub 建议

建议把这个仓库设为私有仓库，因为里面可能包含：

- 私人偏好
- 项目背景
- 工作计划
- 个人决策记录

## 当前定位

这是一个“你拥有的个人 context 系统”，不是绑定某个平台的聊天记录备份工具。

当前已经形成几类稳定用途：

1. 跨模型、跨 agent 的长期上下文
2. 个人画像、协作偏好和表达风格校准
3. 技术中心研发管理计划和课题档案
4. 管道数字化与核心课题母版
5. 资料导入、证据保存和主题主档案更新

下一步更适合继续补：

1. `/load-context` 开始工作流
2. 研发课题组合底账
3. 成果材料、研发计划和表达风格的可复用模板
